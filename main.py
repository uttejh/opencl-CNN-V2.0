...
for iterat_epoch in range(epochs):
	...
	for iterat_image in range(10):
		...
		bias_err = numpy.sum(error)
		while bias_err>1. or bias_err<-1.:
			bias_err = bias_err/10
		...
		for i in range(numOfOutputNeurons):
			...
			for j in range(numOfHiddenNeurons):
				...
				ed = error[i]*derivative
				# E*f`(x)*w 
				Dw_HL_to_output = (ed*weights_HL_to_output[i][j])
				...
				weights_HL_to_output[i][j] += alpha*ed*HL_values[j]
		...
		bias_err_fc = numpy.sum(temp_FC_err)
		while bias_err_fc>1. or bias_err_fc<-1.:
			bias_err_fc = bias_err_fc/10

		# bias update
		bFC -= alpha*bias_err_fc
		...
		bias_err_c2 = numpy.sum(globerrintoder)
		while bias_err_c2>1. or bias_err_c2<-1.:
			bias_err_c2 = bias_err_c2/10

		# bias update
		b2 -= alpha*bias_err_c2
		...
		bias_err_c1 = numpy.sum(globerrintoder_c1)
		while bias_err_c1>1. or bias_err_c1<-1.:
			bias_err_c1 = bias_err_c1/10

		# bias update
		b1 -= alpha*bias_err_c1
...
pickle.dump(weights_HL_to_output, file_HL_to_output)
file_HL_to_output.close()

file_FC_to_HL = open("./weights/FC_to_HL.txt", "w")
pickle.dump(weights_FC_to_HL, file_FC_to_HL)
file_FC_to_HL.close()
